 ...
 */
double
Distance::compute_distance(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud_a_ptr,
                           pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud_b_ptr)
{
  ...
  for (auto point_iter_a = cloud_a.begin(); point_iter_a != cloud_a.end();
       ++point_iter_a) {
    pcl::PointXYZ point_a = *point_iter_a;
    min_sofar = std::numeric_limits<float>::max();
    for (auto point_iter_b = cloud_b.begin(); point_iter_b != cloud_b.end();
         ++point_iter_b) {
...
/**
 * Serial implementation of bi-directional Chamfer distance with k-nearest neighbors
 * instead of all-pairs. Uses a KD-tree structure for KNN queries.
 *
 * In principle, when comparing two point clouds if you can make assumptions on the
 * orientation than you can simply observe that for each point in cloud A we do not have
 * to iterate over all the points in cloud B and instead can search locally in cloud B.
 * Using a KD-tree we can efficiently do k-nearest neighbor searches for each point in A
 * and only compute the distance for those in the returned set in B.
 *
 * @param cloud_a_ptr
 * @param cloud_b_ptr
                           ...
                           int k)
{
  ...
  std::vector<int> pointIdxKNNSearch(
      k); // holds the resultant indices of the neighboring points
  std::vector<float> pointKNNSquaredDistance(
      k); // holds the resultant squared distances to nearby points
  ...
  for (auto point_iter_a = cloud_a.begin(); point_iter_a != cloud_a.end();
    ...
    if (kdtree_b.nearestKSearch(
            point_a, k, pointIdxKNNSearch, pointKNNSquaredDistance) > 0) {
  ...
  for (auto point_iter_b = cloud_b.begin(); point_iter_b != cloud_b.end();
    ...
    if (kdtree_a.nearestKSearch(
            point_b, k, pointIdxKNNSearch, pointKNNSquaredDistance) > 0) {
